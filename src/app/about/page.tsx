      ...
      <article className="max-w-[680px]">
        ...
        <div className="mb-4 grid gap-2">
          {(Object.keys(pillars) as Array<keyof typeof pillars>).map((id) => (
            <Link key={id} href={pillars[id].path} className="kp-spoke">
              <strong>{pillars[id].title}</strong>
              <span className="shrink-0 text-xs text-muted">Hub</span>
            </Link>
          ))}
        </div>
        ...
        <p className="text-sm text-muted">
          Find us on YouTube:{" "}
          <a href={site.youtube} className="text-accent hover:underline" rel="noopener noreferrer">
            @explainstudio9
          </a>{" "}
